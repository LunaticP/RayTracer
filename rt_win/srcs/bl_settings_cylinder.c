...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   bl_settings_cylinder.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jogarcia <jogarcia@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/25 22:18:09 by jogarcia          #+#    #+#             */
/*   Updated: 2017/05/19 21:53:23 by jogarcia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	button_list(t_obj *obj, t_mmlx *win, float *pitch)
{
	...
	i = 0;
	p = (t_pos){.x = XPOS_BUTTON_START, .y = YPOS_CYLINDER_START};
	if (BUTTON_DIFF_Y > 0)
		p.y += BUTTON_DIFF_Y;
	if (BUTTON_DIFF_X > 0)
		p.x += BUTTON_DIFF_X;
	while (i < NUM_CYLINDER_BUTTON)
	{
		value = (cl_float*)((uint64_t)obj + g_rt_offset_button_cylinder[i]);
														...
														t_stringput *value_tab)
{
	...
	pos = (t_pos){.x = XPOS_STR_START, .y = YPOS_CYLINDER_START};
	i = 0;
	while (i < NUM_CYLINDER_PARAM)
	{
		data = name_tab[i];
		data.f_data = (void*)((uint64_t)obj + g_rt_offset_button_cylinder[i]);
		data.xpos = pos.x;
		data.ypos = pos.y;
...
int			bl_settings_cylinder(t_obj *obj, t_mmlx *win, float *pitch)
{
	...
	if (rt_win_error(__func__, string_list(obj, win,
		(t_stringput*)g_rt_strname_cylinder,
		(t_stringput*)g_rt_strvalue_cylinder)))
		return (-1);
	return (0);
}
