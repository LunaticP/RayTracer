...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/25 22:19:10 by jogarcia          #+#    #+#             */
/*   Updated: 2017/05/19 22:07:00 by jogarcia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	button_list(t_obj *obj, t_mmlx *win, float *pitch)
{
	...
	i = 0;
	p = (t_pos){.x = XPOS_BUTTON_START, .y = YPOS_CONE_START};
	if (BUTTON_DIFF_Y > 0)
		p.y += BUTTON_DIFF_Y;
	if (BUTTON_DIFF_X > 0)
		p.x += BUTTON_DIFF_X;
														...
														t_stringput *value_tab)
{
	...
	pos = (t_pos){.x = XPOS_STR_START, .y = YPOS_CONE_START};
	i = 0;
	while (i < NUM_CONE_PARAM)
	{
		data = name_tab[i];
		data.f_data = (void*)((uint64_t)obj + g_rt_offset_button_cone[i]);
		data.xpos = pos.x;
		data.ypos = pos.y;
...
int			bl_settings_cone(t_obj *obj, t_mmlx *win, float *pitch)
{
	...
	if (rt_win_error(__func__, string_list(obj, win,
		(t_stringput*)g_rt_strname_cone, (t_stringput*)g_rt_strvalue_cone)))
		return (-1);
	return (0);
}
